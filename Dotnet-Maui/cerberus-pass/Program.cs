...
if (manager.IsFirstStart) // first start
{
    // does not exist
    ...
    manager.SetupMasterPassword(userInput);
...
{// "masterpass.txt" exists:
 // prompt user for master-password
    ...
    if (
        ...
        && manager.VerifyMasterPassword(userInput)
    )
    {
...
bool PromptCheckMasterPass(out string masterPassPlain)
{
    ...
    if (string.IsNullOrEmpty(masterPassPlain))
        return false;
    return manager.VerifyMasterPassword(masterPassPlain);
}
